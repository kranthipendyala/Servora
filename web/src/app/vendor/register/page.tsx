...
export default function VendorRegisterPage() {
  ...
  // Dynamic data
  const [cities, setCities] = useState<{ id: number; name: string; state_name?: string }[]>([]);
  ...
  // Fetch cities when reaching register step
  useEffect(() => {
    if (step !== "register") return;
    setLoadingData(true);
    fetch("/proxy-api/cities").then((r) => r.json()).then((res) => {
  ...
  const handleRegister = async (e: FormEvent) => {
    ...
    if (!cityId) { setFormError("Please select your city"); return; }
        ...
        <div className="w-full lg:w-1/2 xl:w-[45%] flex items-center justify-center px-4 py-12">
          <div className="w-full max-w-md">
            ...
            <div className="lg:hidden text-center mb-8">
              ...
              <p className="text-slate-400 mt-2 text-sm">Register in 30 seconds</p>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-2xl p-8">
              {step === "phone" && (
                <>
                  <h2 className="text-xl font-semibold text-gray-900 mb-1">Create Vendor Account</h2>
              ...
              {step === "otp" && (
                <>
                  <button type="button" onClick={() => { setStep("phone"); setOtpError(""); }} className="flex items-center gap-1 text-sm text-gray-500 hover:text-gray-700 mb-4 transition-colors">
                  ...
                  <OtpInput length={4} onComplete={handleVerifyOtp} onResend={handleResendOtp} loading={loading} error={otpError} phone={phone} />
                </>
              )}
              ...
              {step === "register" && (
                <>
                  ...
                  <p className="text-sm text-gray-500 mb-6">Just 3 fields — you'll set up categories, services & areas during onboarding</p>
                  ...
                  <form onSubmit={handleRegister} className="space-y-4">
                    ...
                    <div>
                      ...
                      <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="vendor@example.com" className={inputCls} />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">Business Name *</label>
                      <input type="text" value={businessName} onChange={(e) => setBusinessName(e.target.value)} placeholder="e.g. Sharma Electrical Services" className={inputCls} />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">City *</label>
                      <select value={cityId} onChange={(e) => setCityId(Number(e.target.value))} className={`${inputCls} bg-white`}>
